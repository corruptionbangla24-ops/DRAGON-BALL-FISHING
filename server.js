...
// 🛫 ২. ড্রাগন বল ফিশিং কোর এনার্জি বল ফায়ার রাউট (POST Route - ৯৫% জেনুইন RTP ও ডাবল-ডেবিট ব্লকার বর্ম)
app.post('/api/fishing-shoot', async (req, res) => {
    const { userId, amount, wallet, targetTargetId } = req.body; // targetTargetId: কোন ড্রাগন বা জীবে গুলি মারলো
    ...
    if (reqAmount < 1 || reqAmount > 20000) {
        return res.json({ success: false, message: "🚨 Invalid Energy Parameter! Max 20000 ৳" });
    }
    ...
    try {
        ...
        // 🎰 [🎰 আন্তর্জাতিক জেনুইন র্যান্ডম ৯৫% RTP শুটার লুপ ইঞ্জিন ভাই ভাই]
        while (isLoopActive && loopSafety < 150) {
            loopSafety++;
            
            // এনার্জি বল ফায়ার করলে ড্রাগন ব্লাস্ট হবে কি হবে না তার র্যান্ডম চান্স প্রোটোকল
            let hitChance = Math.random();
            
            if (hitChance <= 0.26) { // ২৬% স্বাভাবিক চান্স থাকবে ড্রাগন টার্গেট ওয়ান-শটে ধ্বংস করার!
                isBlasted = true;
                finalStatus = "win";
        ...
        return res.json({ success: false, message: "⚠️ Timeout! Tap to fire energy ball again." }); 
    }
});
